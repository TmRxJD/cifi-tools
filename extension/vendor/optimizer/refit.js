// HOW A DONOR BUILD IS RE-FITTED TO A DIFFERENT BUDGET. Shipped copy of the canonical logic.
//
// Kept in ONE place: tools/bench/refit.js is generated from this by nothing -- they are the same
// algorithm and must not drift. `refit-parity-check.js` asserts the two produce identical output on
// every fixture, because two copies of a rule is exactly the shape this project has watched rot.
(function (global) {
  'use strict';
  ...
  function gatePayingFill(defs, minVal, budget, opts) {
    ...
    // DEPS MUST BE PASSED EXPLICITLY -- a silent `|| {}` re-arms the bug described above.
    //
    // Omitting them does not fail loudly, it produces a LEGAL-LOOKING allocation that violates
    // dependency edges, and the comment above records that costing 24 of 24 fixtures and nearly
    // poisoning the experiment this function was written for. An absent `deps` is a caller mistake;
    // a genuinely dependency-free space (every talent block) passes `{}` and says so.
    if (!opts || opts.deps === undefined || opts.deps === null) {
      throw new Error('gatePayingFill: opts.deps is required -- pass {} explicitly for a '
        + 'dependency-free space. Defaulting it silently produces builds that violate dependency '
        + 'edges while looking legal.');
    }
  ...
  global.OptimizerRefit = { refitNaive, refitTiered, refitTalents, gatePayingFill, capOf, spend, pointsBelow };

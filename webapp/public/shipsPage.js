...
// How many trailing Install Order steps to show in full. ONE definition, used by both the reader
// and the writer, so a value that round-trips through the store cannot mean something different on
// the way back out.
//
// Anything unusable becomes 0 = SHOW ALL, deliberately. The failure this avoids is the silent-zero
// trap in reverse: if a junk value collapsed the list to a tiny tail, the user would be shown a
// SHORTER list than they asked for and could easily read it as the whole plan. Falling back to
// "show everything" can only ever be too verbose, never misleading.
function clampShowLast(value) {
...
function expandClicks(clicks, catalog, startLevels, shipId) {
  ...
  return clicks.map((slot) => {
    ...
    return {
      // `slot` is carried so a consumer can GROUP by node, not just print it. The "Show last N"
      // summary needs node identity; names are display text and two nodes may legitimately read
      // alike once truncated.
      slot,
...
function openLoadoutDetail(shipId, levels, mode, clicks) {
  ...
  // purpose, which is knowing what to click NEXT. The ORDER only carries information where nodes
  // are still interleaving; everything before the tail can be bought in any order and reaches the
  // same state, so it is shown as a per-node TOTAL -- exactly the bulk-buy targets -- and only the
  // tail stays sequential.
  //
  // The prefix summary is a state, not a sequence, so it is read from the LAST step of each node in
  // the prefix rather than by counting steps: `expandClicks` already carries the running level, and
  ...
  const showLastInput = document.getElementById('installOrderShowLast');
  if (showLastInput) {
    showLastInput.onchange = () => {
      window.store.installOrderShowLast = clampShowLast(showLastInput.value);
      window.saveStore();
      openLoadoutDetail(shipId, levels, mode, clicks);
    };
  }

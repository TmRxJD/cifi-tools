...
// LEVEL against markup that lives in index.html, so the extension must supply them (see SHELL_IDS)
...
const SHELL_IDS = [
  'shipBuildModal', 'newLoadoutModal', 'optimizeShipModal',
  'zaglagChecklistModal', 'loadoutDetailModal',
];
...
function buildShell() {
  ...
  const markup = SHELL_IDS.map((id) => extractById(html, id)).join('\n');
...
if (process.argv.includes('--check')) {
  ...
  console.log(`wrote extension/shell.js  (${SHELL_IDS.length} modals, ${shell.length} bytes)`);
